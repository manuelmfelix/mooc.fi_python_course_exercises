...
def cheaper_properties(properties: list, reference: RealProperty):
    return[(properties[a],properties[a].price_difference(reference)) for a in range (0, len(properties)) if reference.more_expensive(properties[a])]
...
if __name__ == '__main__':
    ...
    # for item in cheaper_properties([RealProperty(1,1,1,"a")], RealProperty(1,1,2,"b")):
    for item in cheaper_properties(properties, a3):
        print(f"{item[0].description:35} price difference {item[1]} euros")
    # [print (properties[a].description,properties[a].price_difference(a3)) for a in range (0, len(properties)-1)]